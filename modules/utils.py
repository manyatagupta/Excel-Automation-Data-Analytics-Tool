...
def create_download_link(file_content, filename, text):
    """Generates a link to download the given content."""
    b64 = base64.b64encode(file_content).decode()
    href = f'<a href="data:file/txt;base64,{b64}" download="{filename}">{text}</a>'
    return href

def create_excel_download_link(df, filename="data.xlsx", text="Download Excel"):
    """Generates a Streamlit download button for Excel."""
    output = BytesIO()
